...
"""
def random_name(split=' ',lowercase=False):
    ALL_ENG_NAMES = [
...
"""
randow get a sex male(60%) or female(40%)
"""
def randow_sex():
    list = ("male","male","male","female","female")
...
def random_birthday():
    birthday = str(random.randint(1960, 2000))  # 年份项
    da = datetime.date.today() + datetime.timedelta(days=random.randint(1, 366))
    birthday = birthday + da.strftime('-%m-%d')
    return birthday

def random_address():
    global address_dic
    address_dic = {'01': '新洲', '02': '蔡甸', '03': '武昌', '04': '江岸', '05': '江夏'}
    address_list = ["01","01","01","02","03","04","05"]
    address_code = random.choice(address_list)
    return address_code

def random_answer():
    answer_list = ["a","b","b","b","b","b","c","c"]
    return random.choice(answer_list)

def makr_rowkey(phoneNo,address_code):
    rowkey = ""
    rowkey = rowkey + phoneNo+address_code
    for i in range(1,4):
        rowkey = rowkey + str(random.randint(0,9))
    return rowkey
